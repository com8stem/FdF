...
/*   Updated: 2023/12/04 20:04:06 by kishizu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	draw_wireframe(t_info *fdf, t_data *img)
{
	...
	while (points < fdf->points)
	{
		...
		{
			drawline((tmp_x * ENLARGERATE) + INIMOVE, (tmp_y * ENLARGERATE) + INIMOVE,
	 				(fdf->xyz[points + 1].x * ENLARGERATE) + INIMOVE, (fdf->xyz[points + 1].y * ENLARGERATE) + INIMOVE, img);
		}
		if ((points + 1) / fdf->x_len != fdf->y_len - 1)
		{
			drawline((tmp_x * ENLARGERATE) + INIMOVE, (tmp_y * ENLARGERATE) + INIMOVE,
 					(fdf->xyz[points + fdf->x_len].x * ENLARGERATE) + INIMOVE, (fdf->xyz[points + fdf->x_len].y * ENLARGERATE) + INIMOVE, img);
		}
		points++;
		printf("[%d][%d]\n", points, fdf->points);
	}
